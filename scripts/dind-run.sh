...
for optional in DSH_PROVIDER DSH_PERMISSION_MODE DSH_CONTEXT_WINDOW DSH_TEMPERATURE DSH_TOP_P DSH_SDK_MINIMAL_MAX_TOKENS; do
  if [[ ${!optional+x} ]]; then
    run_env+=("$optional=${!optional}")
  fi
done
for optional in PIP_INDEX_URL PIP_EXTRA_INDEX_URL PIP_TRUSTED_HOST UV_INDEX_URL UV_DEFAULT_INDEX NPM_CONFIG_REGISTRY HARBOR_APT_UBUNTU_MIRROR HARBOR_APT_DEBIAN_MIRROR HARBOR_APT_DEBIAN_SECURITY_MIRROR GO111MODULE GOPROXY GOSUMDB RUSTUP_UPDATE_ROOT RUSTUP_DIST_SERVER CARGO_REGISTRY_REPLACE_WITH CARGO_REGISTRY_URL DIND_REGISTRY_MIRRORS DIND_REGISTRY_MIRROR HARBOR_SKIP_DOCKERHUB_PREFLIGHT; do
  if [[ ${!optional+x} ]]; then
    run_env+=("$optional=${!optional}")
  fi
done
