...
_DEFAULT_EXCLUDED_PORTS = frozenset({18100})
...
def capture_new_listeners(
    baseline: set[ProcessIdentity],
    manifest_path: Path,
    *,
    proc_root: Path = Path("/proc"),
    excluded_ports: frozenset[int] = _DEFAULT_EXCLUDED_PORTS,
) -> list[ProcessRecord]:
    """Persist new TCP listeners that may be removed by DSH shutdown."""
    selected = _new_listener_records(
        baseline,
        proc_root=proc_root,
    ...
    *,
    ...
    for identity in snapshot_processes(proc_root):
        ...
        for inode in _process_socket_inodes(identity.pid, proc_root):
            ...
            if port is not None and port not in excluded_ports:
...
def watch_new_listeners(
    ...
    *,
    ...
    Per-process last-seen times keep services removed by the teardown while
    excluding development servers the agent intentionally stopped earlier.
    """
    baseline = snapshot_processes(proc_root)
    observed: dict[tuple[int, str], tuple[ProcessRecord, float]] = {}
    while control_path.exists():
        now = time.monotonic()
        for record in _new_listener_records(
            baseline,
            proc_root=proc_root,
            excluded_ports=excluded_ports,
        ):
...
def restore_services(
    ...
    *,
    ...
    for index, record in enumerate(records):
        ...
        if _identity_alive(record, proc_root):
        ...
        while time.monotonic() < deadline:
            ...
            if process.poll() not in (None, 0):
                summary.update(
                    status="exited-before-ready", exit_code=process.returncode
                )
                break
            time.sleep(0.1)
        else:
