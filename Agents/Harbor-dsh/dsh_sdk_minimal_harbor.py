...
class AgentFleetDshSdkMinimal(BaseInstalledAgent):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if provider_route != "deepseek":
            raise ValueError("dsh-sdk-minimal supports the native DeepSeek provider only")
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        version = str(
            self.version()
            or os.environ.get("DSH_SDK_MINIMAL_DSH_VERSION", "0.1.2-alpha.2")
        )
        ...
        await self.exec_as_root(
            ...
            env={
                ...
                or "/opt/tb-opik/python-wheels/dsh-minimal-python3.12-runtime.tar.gz",
                ...
                or "/opt/tb-opik/python-wheels/node-runtime.tar.gz",
                "DSH_RUNTIME_TAR_PATH": self._get_env("DSH_RUNTIME_TAR_PATH")
                or f"/opt/tb-opik/python-wheels/dsh-runtime-{version}.tar.gz",
            },
        )
        await self.exec_as_agent(
            environment,
            command=(
                ...
                'tar -xzf "${DSH_RUNTIME_TAR_PATH}" -C "$HOME/.local"; '
                'export PATH="$HOME/.local/bin:$PATH"; '
