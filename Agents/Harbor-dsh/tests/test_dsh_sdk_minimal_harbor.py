...
class AgentFleetDshSdkMinimalTests(unittest.IsolatedAsyncioTestCase):
    def make_agent(self, root: Path, **kwargs: Any) -> AgentFleetDshSdkMinimal:
        return AgentFleetDshSdkMinimal(
            ...
            extra_env={
                ...
                "DSH_RUNTIME_TAR_PATH": "/cache/dsh.tar.gz",
            },
            **kwargs,
        )
    ...
    def test_runner_passes_all_version_matched_profile_fields(self) -> None:
        content = (MODULE_DIR / "dsh_sdk_minimal_runner.py").read_text(
            encoding="utf-8"
        )
        for field in (
        ...
        self.assertNotIn("cordis=", content)
        self.assertNotIn("session_root=", content)
