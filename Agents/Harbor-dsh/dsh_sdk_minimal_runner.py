...
def request_service_snapshot(
    request_path: Path | None,
    ready_path: Path | None,
    *,
    timeout: float = 10.0,
) -> None:
    """Freeze listeners that are alive immediately before harness teardown."""
    if request_path is None and ready_path is None:
        return
    if request_path is None or ready_path is None:
        raise ValueError("service snapshot request and ready paths must be paired")
    request_path.touch()
    ...
    while time.monotonic() < deadline:
        if ready_path.is_file():
            return
        time.sleep(0.05)
    raise TimeoutError("service handoff watcher did not acknowledge snapshot")
...
def main() -> None:
    ...
    parser.add_argument("--service-snapshot-request", type=Path)
    parser.add_argument("--service-snapshot-ready", type=Path)
    ...
    with trace_path.open("w", encoding="utf-8") as trace:
        ...
        with DeepSeekHarness(
            ...
            request_service_snapshot(
                args.service_snapshot_request,
                args.service_snapshot_ready,
            )
        trace.write(
            json.dumps(
                {
