...
| `HARBOR_OPENSANDBOX_EXISTING_IMAGE_MAP_FILE` | Optional JSON task-to-digest map for verified immutable images already present in each configured Harbor task repository |
...
| `YICLOUD_SANDBOX_COMMAND_READY_TIMEOUT_SEC` | Bounded retry window for transient `403`/`429`/`502`/`503`/`504` responses while a new Sandbox command proxy becomes ready, default `90` |
| `YICLOUD_SANDBOX_DIAGNOSTICS_DIR` | Optional host directory for token-free per-Sandbox JSON records used by exact-ID runtime monitoring |
