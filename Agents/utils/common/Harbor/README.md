...
```bash
AGENT="claude-code"        # claude-code, opencode, pi, or dsh
DATASET_NAME="seta"        # built-in Harbor registry alias
TOTAL_WORKERS="80"
HARBOR_N_CONCURRENT="80"
```

For OpenCode or DSH fixed benchmark runs, optional generation controls can be set in
...
`HARBOR_MAX_TOKENS` also applies to Claude Code, Pi, and DSH. Claude Code and
Pi expose neither temperature nor top-p, while DSH 0.1.1-rc.2 exposes
temperature but not top-p. The runner rejects unsupported settings instead of
silently ignoring them. The Pi
