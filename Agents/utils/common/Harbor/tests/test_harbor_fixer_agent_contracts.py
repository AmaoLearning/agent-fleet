...
from harbor_fixer.plan_generation import request_fix_plan  # noqa: E402
from harbor_fixer.prompts import (  # noqa: E402
    MAIN_AGENT_PROMPT,
    REPORT_MAIN_AGENT_PROMPT,
    TASK_SUBAGENT_PROMPT,
)
from harbor_fixer.reporter import generate_report_summary  # noqa: E402
from harbor_fixer.validation import validate_fix_plan_set  # noqa: E402
...
class HarborFixerAgentContractsTest(FixerTestCase):
    def test_agent_prompts_define_complete_output_contracts(self) -> None:
        ...
        self.assertIn(
            "The output field is named plans, never fix_plans", MAIN_AGENT_PROMPT
        )
        ...
        self.assertIn(
            "status describes summary generation, not the verification outcome",
            REPORT_MAIN_AGENT_PROMPT,
        )
    ...
    def test_fix_plan_and_report_retries_include_schema_feedback(self) -> None:
        ...
        valid_plan = make_fix_plan()
        valid_plan["source"] = {}
        valid_plan["plans"] = []
        plan_invoker = SequenceInvoker([invalid_plan, json.dumps(valid_plan)])
        ...
        self.assertIn("generation_errors must be empty", plan_invoker.records[1][0])
