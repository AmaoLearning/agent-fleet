...
from fixer_test_support import (
    ...
    make_exec_result,
    make_fix_plan,
    write_harbor_run_fixture,
    write_json,
    write_smoke_rerun_script,
    write_verification_inputs,
)
...
class HarborFixerVerificationTest(FixerTestCase):
    def run_smoke(self, statuses: dict[str, str], *, count: int = 4, limit: int = 2):
        output, plan, execution = write_verification_inputs(
            self.root,
            count=count,
        )
    ...
    def test_verification_smoke_samples_two_and_marks_unsampled(self) -> None:
        ...
        statuses = [item["verification_status"] for item in result["task_results"]]
        ...
        self.assertTrue(
            (self.root / "fixer" / "verification-smoke-selection.json").exists()
        )
