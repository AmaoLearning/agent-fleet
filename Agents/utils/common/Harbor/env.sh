...
DSH_MINIMAL_SDK_VERSION="${DSH_MINIMAL_SDK_VERSION:-0.1.0-rc.6}"
DSH_MINIMAL_RUNTIME_BASENAME="${DSH_MINIMAL_RUNTIME_BASENAME:-dsh-minimal-runtime-${DSH_MINIMAL_SDK_VERSION}.tar.gz}"
DSH_MINIMAL_RUNTIME_VERSION_BASENAME="${DSH_MINIMAL_RUNTIME_VERSION_BASENAME:-dsh-minimal-runtime.version}"
DSH_MINIMAL_PYTHON_RUNTIME_BASENAME="${DSH_MINIMAL_PYTHON_RUNTIME_BASENAME:-dsh-minimal-python3.12-runtime.tar.gz}"
DSH_MINIMAL_MAX_TOKENS="${DSH_MINIMAL_MAX_TOKENS:-}"
DSH_SDK_MINIMAL_DSH_VERSION="${DSH_SDK_MINIMAL_DSH_VERSION:-0.1.2-alpha.2}"
...
DSH_SDK_MINIMAL_DSH_RUNTIME_BASENAME="${DSH_SDK_MINIMAL_DSH_RUNTIME_BASENAME:-dsh-runtime-${DSH_SDK_MINIMAL_DSH_VERSION}.tar.gz}"
DSH_SDK_MINIMAL_DSH_RUNTIME_VERSION_BASENAME="${DSH_SDK_MINIMAL_DSH_RUNTIME_VERSION_BASENAME:-dsh-sdk-minimal-dsh-runtime.version}"
DSH_SDK_MINIMAL_MAX_TOKENS="${DSH_SDK_MINIMAL_MAX_TOKENS:-}"
...
if [[ "$AGENT" == "dsh-minimal" && -z "$HARBOR_AGENT_IMPORT_PATH" ]]; then
  HARBOR_AGENT_IMPORT_PATH="dsh_minimal_harbor:AgentFleetDshMinimal"
fi
...
if [[ -z "${DSH_BASE_URL:-}" && -n "${HARBOR_ANTHROPIC_BASE_URL:-}" ]]; then
  DSH_BASE_URL="${HARBOR_ANTHROPIC_BASE_URL%/}"
  DSH_BASE_URL="${DSH_BASE_URL%/chat/completions}"
  DSH_BASE_URL="${DSH_BASE_URL%/responses}"
  if [[ "$DSH_BASE_URL" != */v1 ]]; then
    DSH_BASE_URL="${DSH_BASE_URL}/v1"
  fi
fi
...
export DSH_VERSION DSH_RUNTIME_BASENAME DSH_RUNTIME_VERSION_BASENAME DSH_MINIMAL_SDK_VERSION DSH_MINIMAL_RUNTIME_BASENAME DSH_MINIMAL_RUNTIME_VERSION_BASENAME DSH_MINIMAL_PYTHON_RUNTIME_BASENAME DSH_MINIMAL_MAX_TOKENS DSH_SDK_MINIMAL_DSH_VERSION DSH_SDK_MINIMAL_SOURCE_REF DSH_SDK_MINIMAL_SOURCE_SHA DSH_SDK_MINIMAL_SOURCE_DIR DSH_SDK_MINIMAL_RUNTIME_BASENAME DSH_SDK_MINIMAL_RUNTIME_VERSION_BASENAME DSH_SDK_MINIMAL_DSH_RUNTIME_BASENAME DSH_SDK_MINIMAL_DSH_RUNTIME_VERSION_BASENAME DSH_SDK_MINIMAL_MAX_TOKENS DSH_PROVIDER DSH_THINKING_FORMAT DSH_PERMISSION_MODE DSH_THINKING DSH_REASONING_EFFORT DSH_TEMPERATURE DSH_MAX_TOKENS DSH_CONTEXT_WINDOW DSH_PROVIDER_RETRY_MAX DSH_PROCESS_RETRY_MAX DSH_REQUEST_TIMEOUT_MS DSH_STREAM_IDLE_TIMEOUT_MS DSH_API_KEY DSH_BASE_URL
export HARBOR_LOCAL_WHEEL_SERVER_URL HARBOR_LOCAL_CLAUDE_TGZ_URL HARBOR_REMOTE_WHEEL_SERVER_URLS EFFECTIVE_WHEEL_URL_FILE EFFECTIVE_CLAUDE_TGZ_URL_FILE LOCAL_DEPS_LOG_FILE HARBOR_RUNNER_PREPARE HARBOR_RUNNER_IMAGE_DIR HARBOR_RUNNER_HOST_DIR HARBOR_RUNNER_PYTHON_VERSION HARBOR_RUNNER_DIR HARBOR_OPIK_BIN HARBOR_CLI_BIN HARBOR_OPIK_PYTHON HARBOR_RUNNER_REQUIREMENTS HARBOR_RUNNER_SOURCE_COMMIT HARBOR_RUNNER_PREPARE_STATUS_FILE HARBOR_RUNNER_PREPARE_LOG_FILE
...
harbor_agent_is_dsh() {
  [[ "$AGENT" == "dsh" || "$AGENT" == "dsh-minimal" || "$AGENT" == "dsh-sdk-minimal" ]]
}

harbor_agent_is_dsh_minimal() {
  [[ "$AGENT" == "dsh-minimal" ]]
}

harbor_agent_is_dsh_sdk_minimal() {
...
harbor_validate_agent() {
  case "$AGENT" in
    claude-code|opencode|pi|dsh|dsh-minimal|dsh-sdk-minimal|oracle) ;;
    *)
      echo "[ERROR] AGENT must be claude-code, opencode, pi, dsh, dsh-minimal, dsh-sdk-minimal, or oracle, got: $AGENT" >&2
      exit 1
      ;;
  esac
  ...
  if harbor_agent_is_dsh; then
    ...
    if { harbor_agent_is_dsh_minimal || harbor_agent_is_dsh_sdk_minimal; } \
      && [[ "$DSH_PROVIDER" != "deepseek" ]]; then
      echo "[ERROR] DSH minimal profiles support only DSH_PROVIDER=deepseek." >&2
      exit 1
    fi
    if harbor_agent_is_dsh_minimal && [[ -n "$DSH_MINIMAL_MAX_TOKENS" ]]; then
      if [[ ! "$DSH_MINIMAL_MAX_TOKENS" =~ ^[1-9][0-9]*$ ]]; then
        echo "[ERROR] DSH_MINIMAL_MAX_TOKENS must be a positive integer." >&2
        exit 1
      fi
    fi
    if harbor_agent_is_dsh_sdk_minimal && [[ -n "$DSH_SDK_MINIMAL_MAX_TOKENS" ]]; then
      if [[ ! "$DSH_SDK_MINIMAL_MAX_TOKENS" =~ ^[1-9][0-9]*$ ]]; then
...
harbor_ensure_local_wheels_server() {
  ...
  for port in $(seq "$LOCAL_WHEEL_PORT" "$last_port"); do
    ...
    if harbor_agent_is_opencode; then
      ...
      urls+=("${HARBOR_LOCAL_WHEEL_SERVER_URL%/}/${DSH_MINIMAL_RUNTIME_VERSION_BASENAME}")
      urls+=("${HARBOR_LOCAL_WHEEL_SERVER_URL%/}/${DSH_MINIMAL_PYTHON_RUNTIME_BASENAME}")
    elif harbor_agent_is_dsh; then
  ...
  [[ -f "$LOCAL_WHEEL_DIR/manifest.txt" ]] \
    ...
    && {
      if harbor_agent_is_opencode; then
        ...
        harbor_tar_file_ready "$LOCAL_WHEEL_DIR/${DSH_MINIMAL_RUNTIME_BASENAME}" \
          && harbor_tar_file_ready "$LOCAL_WHEEL_DIR/${DSH_MINIMAL_PYTHON_RUNTIME_BASENAME}" \
          && [[ -f "$LOCAL_WHEEL_DIR/${DSH_MINIMAL_RUNTIME_VERSION_BASENAME}" ]] \
          && grep -qx "$DSH_MINIMAL_SDK_VERSION" "$LOCAL_WHEEL_DIR/${DSH_MINIMAL_RUNTIME_VERSION_BASENAME}"
      elif harbor_agent_is_dsh_sdk_minimal; then
        harbor_tar_file_ready "$LOCAL_WHEEL_DIR/${DSH_MINIMAL_PYTHON_RUNTIME_BASENAME}" \
          && harbor_tar_file_ready "$LOCAL_WHEEL_DIR/${DSH_SDK_MINIMAL_RUNTIME_BASENAME}" \
          && grep -qx "${DSH_SDK_MINIMAL_SOURCE_REF}@${DSH_SDK_MINIMAL_SOURCE_SHA}" \
            "$LOCAL_WHEEL_DIR/${DSH_SDK_MINIMAL_RUNTIME_VERSION_BASENAME}" \
          && harbor_tar_file_ready "$LOCAL_WHEEL_DIR/node-runtime.tar.gz" \
          && harbor_tar_file_ready "$LOCAL_WHEEL_DIR/${DSH_SDK_MINIMAL_DSH_RUNTIME_BASENAME}" \
          && grep -qx "$DSH_SDK_MINIMAL_DSH_VERSION" \
            "$LOCAL_WHEEL_DIR/${DSH_SDK_MINIMAL_DSH_RUNTIME_VERSION_BASENAME}"
      elif harbor_agent_is_dsh; then
        harbor_tar_file_ready "$LOCAL_WHEEL_DIR/node-runtime.tar.gz" \
...
harbor_prepare_or_select_wheels() {
  ...
  if (cd "$SCRIPT_DIR" && WHEEL_DIR="$LOCAL_WHEEL_DIR" CACHE_SCHEMA=3 CLAUDE_CODE_VERSION="$CLAUDE_CODE_VERSION" CLAUDE_CODE_TGZ_BASENAME="$CLAUDE_CODE_TGZ_BASENAME" PREPARE_OPENCODE_CACHE="$prepare_opencode_cache" OPENCODE_VERSION="$OPENCODE_VERSION" OPENCODE_TGZ_BASENAME="$OPENCODE_TGZ_BASENAME" OPENCODE_LINUX_X64_TGZ_BASENAME="$OPENCODE_LINUX_X64_TGZ_BASENAME" PREPARE_PI_CACHE="$prepare_pi_cache" PI_VERSION="$PI_VERSION" PI_TGZ_BASENAME="$PI_TGZ_BASENAME" PI_NODE_RUNTIME_BASENAME="$PI_NODE_RUNTIME_BASENAME" PI_RUNTIME_BASENAME="$PI_RUNTIME_BASENAME" ./prepare_local_deps.sh 2>&1 | tee -a "$LOCAL_DEPS_LOG_FILE"); then
    if harbor_agent_is_dsh_sdk_minimal; then
      if ! (cd "$SCRIPT_DIR" && \
        WHEEL_DIR="$LOCAL_WHEEL_DIR" \
        DSH_VERSION="$DSH_SDK_MINIMAL_DSH_VERSION" \
        DSH_RUNTIME_BASENAME="$DSH_SDK_MINIMAL_DSH_RUNTIME_BASENAME" \
        DSH_RUNTIME_VERSION_FILE="$LOCAL_WHEEL_DIR/$DSH_SDK_MINIMAL_DSH_RUNTIME_VERSION_BASENAME" \
        NPM_CONFIG_REGISTRY="$NPM_CONFIG_REGISTRY" \
        python3 ./prepare_dsh_runtime.py 2>&1 | tee -a "$LOCAL_DEPS_LOG_FILE"); then
        echo "failed to prepare DSH sdk-minimal CLI runtime" >&2
        echo "failed" > "$status_file"
        touch "$WORKERS_FAILED_FILE"
        return 1
      fi
      if ! (cd "$REPO_ROOT" && \
        ...
        DSH_MINIMAL_PYTHON_RUNTIME_TARBALL="$LOCAL_WHEEL_DIR/$DSH_MINIMAL_PYTHON_RUNTIME_BASENAME" \
        "$HARBOR_OPIK_PYTHON" -m Agents.utils.common.Harbor.prepare_dsh_sdk_minimal_runtime 2>&1 | tee -a "$LOCAL_DEPS_LOG_FILE"); then
        echo "failed to prepare DSH sdk-minimal Python runtime" >&2
        echo "failed" > "$status_file"
        touch "$WORKERS_FAILED_FILE"
        return 1
      fi
      ...
      if ! (cd "$SCRIPT_DIR" && \
        WHEEL_DIR="$LOCAL_WHEEL_DIR" \
        DSH_MINIMAL_SDK_VERSION="$DSH_MINIMAL_SDK_VERSION" \
        DSH_MINIMAL_RUNTIME_BASENAME="$DSH_MINIMAL_RUNTIME_BASENAME" \
        DSH_MINIMAL_RUNTIME_VERSION_FILE="$LOCAL_WHEEL_DIR/$DSH_MINIMAL_RUNTIME_VERSION_BASENAME" \
        DSH_MINIMAL_PYTHON_RUNTIME_TARBALL="$LOCAL_WHEEL_DIR/$DSH_MINIMAL_PYTHON_RUNTIME_BASENAME" \
        "$HARBOR_OPIK_PYTHON" ./prepare_dsh_minimal_runtime.py 2>&1 | tee -a "$LOCAL_DEPS_LOG_FILE"); then
        echo "failed to prepare DSH minimal runtime" >&2
        echo "failed" > "$status_file"
        touch "$WORKERS_FAILED_FILE"
        return 1
      fi
