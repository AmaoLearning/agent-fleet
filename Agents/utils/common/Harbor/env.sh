...
HARBOR_ANTE_DIR="${HARBOR_ANTE_DIR:-$AGENTS_DIR/Harbor-ante}"
...
# AGENT selects the runner: claude-code (default), opencode, pi, or ante.
...
_HARBOR_ANALYZER_DEFAULT="$HARBOR_MONITOR_ENABLED"
if [[ "$AGENT" == "ante" ]]; then
  _HARBOR_ANALYZER_DEFAULT=0
fi
HARBOR_ANALYZER_ENABLED="${HARBOR_ANALYZER_ENABLED:-$_HARBOR_ANALYZER_DEFAULT}"
...
ANTE_RUNTIME_DIR="${ANTE_RUNTIME_DIR:-$AGENT_FLEET_CACHE_DIR/ante/$ANTE_VERSION}"
ANTE_BINARY_PATH="${ANTE_BINARY_PATH:-$ANTE_RUNTIME_DIR/ante}"
ANTE_PROVIDER="${ANTE_PROVIDER:-openai-compatible}"
ANTE_REASONING_EFFORT="${ANTE_REASONING_EFFORT:-max}"
ANTE_ARGS="${ANTE_ARGS:---yolo --output-format json --no-session-save --no-skills --check}"
ANTE_ENABLE_ATIF="${ANTE_ENABLE_ATIF:-true}"
ANTE_HOME="${ANTE_HOME:-/tmp/ante-home}"
ANTE_MODEL_BASE_URL="${ANTE_MODEL_BASE_URL:-${BASE_URL%/}/v1}"
ANTE_MODEL_CONTEXT_LIMIT="${ANTE_MODEL_CONTEXT_LIMIT:-}"
...
if [[ "$AGENT" == "ante" && -z "$HARBOR_AGENT_IMPORT_PATH" ]]; then
  HARBOR_AGENT_IMPORT_PATH="ante_harbor:AnteAgent"
fi
...
YICLOUD_SANDBOX_COMMAND_READY_TIMEOUT_SEC="${YICLOUD_SANDBOX_COMMAND_READY_TIMEOUT_SEC:-90}"
YICLOUD_SANDBOX_STATUS_LOG_INTERVAL_SEC="${YICLOUD_SANDBOX_STATUS_LOG_INTERVAL_SEC:-30}"
YICLOUD_SANDBOX_DIAGNOSTICS_DIR="${YICLOUD_SANDBOX_DIAGNOSTICS_DIR:-}"
...
export YICLOUD_SANDBOX_COMMAND_READY_TIMEOUT_SEC
export YICLOUD_SANDBOX_STATUS_LOG_INTERVAL_SEC
export YICLOUD_SANDBOX_DIAGNOSTICS_DIR
...
HARBOR_OPENSANDBOX_EXISTING_IMAGE_MAP_FILE="${HARBOR_OPENSANDBOX_EXISTING_IMAGE_MAP_FILE:-}"
...
export SCRIPT_DIR REPO_ROOT AGENTS_DIR TASKS_DIR HARBOR_CLAUDE_CODE_DIR HARBOR_OPENCODE_DIR HARBOR_PI_DIR HARBOR_ANTE_DIR WORKSPACE_DIR RUN_ID TOTAL_WORKERS N_ATTEMPTS MODEL AGENT MAX_RETRIES
...
export ANTE_VERSION ANTE_MANIFEST_URL ANTE_RUNTIME_DIR ANTE_BINARY_PATH ANTE_PROVIDER ANTE_REASONING_EFFORT ANTE_ARGS ANTE_ENABLE_ATIF ANTE_HOME ANTE_MODEL_BASE_URL ANTE_MODEL_CONTEXT_LIMIT
...
harbor_validate_agent() {
  case "$AGENT" in
    claude-code|opencode|pi|ante|oracle) ;;
    *)
      echo "[ERROR] AGENT must be claude-code, opencode, pi, ante, or oracle, got: $AGENT" >&2
      exit 1
      ;;
  esac
  ...
  if harbor_agent_is_ante; then
    case "$ANTE_REASONING_EFFORT" in
      min|low|medium|high|xhigh|max) ;;
      *)
        echo "[ERROR] ANTE_REASONING_EFFORT must be min, low, medium, high, xhigh, or max." >&2
        exit 1
        ;;
    esac
    if [[ -z "$ANTE_PROVIDER" ]]; then
      echo "[ERROR] ANTE_PROVIDER must not be empty." >&2
      exit 1
    fi
  fi
}
...
harbor_prepare_ante_runtime() {
  ANTE_VERSION="$ANTE_VERSION" \
    ANTE_MANIFEST_URL="$ANTE_MANIFEST_URL" \
    ANTE_RUNTIME_DIR="$ANTE_RUNTIME_DIR" \
    ANTE_BINARY_PATH="$ANTE_BINARY_PATH" \
    bash "$HARBOR_ANTE_DIR/prepare_runtime.sh"

  if [[ "$HARBOR_ENVIRONMENT_TYPE" == "opensandbox" \
    && "$YICLOUD_SANDBOX_UPLOAD_BACKEND" == "s3" ]]; then
    echo "prewarming immutable Ante OpenSandbox object..."
    python3 "$SCRIPT_DIR/opensandbox_s3_upload.py" preflight
    python3 "$SCRIPT_DIR/opensandbox_s3_upload.py" prewarm "$ANTE_BINARY_PATH"
  fi
}
...
harbor_prepare_agent_runtime() {
  if harbor_agent_is_ante; then
    mkdir -p "$RUNTIME_DIR"
    rm -f "$WORKERS_READY_FILE" "$WORKERS_FAILED_FILE" "$HARBOR_RUNNER_PREPARE_STATUS_FILE"
    if harbor_validate_runner_cli && harbor_prepare_ante_runtime; then
