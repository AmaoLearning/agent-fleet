"""Prepare a portable, pinned DeepSeek Harness runtime for task sandboxes."""
...
class Config:
    ...
    def from_environment(
        ...
        version = _value(values, "DSH_VERSION", "0.1.1-rc.2")
        runtime_basename = _value(
            values, "DSH_RUNTIME_BASENAME", f"dsh-runtime-{version}.tar.gz"
        )
        return cls(
            wheel_dir=wheel_dir,
            version=version,
            ...
            version_file=Path(
                _value(
                    values,
                    "DSH_RUNTIME_VERSION_FILE",
                    str(wheel_dir / "dsh-runtime.version"),
                )
            ),
            node_runtime_tarball=Path(
                _value(
