...
EventRetention = Literal["raw", "compact"]
...
def _raw_event_stream(path: Path) -> tuple[list[dict[str, Any]], int, dict[str, Any]]:
    raw = path.read_text(encoding="utf-8", errors="replace")
    events, invalid_lines = parse_jsonl(raw)
    return events, invalid_lines, {
        "events_sha256": hashlib.sha256(raw.encode("utf-8")).hexdigest(),
...
def run_pi_json_process(
    *,
    ...
    event_retention: EventRetention = "compact",
    auth_header: bool | None = None,
) -> PiProcessResult:
    ...
    if event_retention not in {"raw", "compact"}:
        return PiProcessResult(None, "", record, "pi_event_retention_invalid", "")
    ...
    raw_events_path = (
        events_path
        if event_retention == "raw"
        else events_path.with_name(f".{events_path.name}.raw")
    )
    try:
        with raw_events_path.open("w", encoding="utf-8") as stdout_file, stderr_path.open(
            ...
            try:
                process.communicate(input=prompt if prompt_in_stdin else None, timeout=timeout_seconds)
                ...
                try:
                    if event_retention == "compact":
                        _, _, event_record = compact_jsonl_event_stream(
                            raw_events_path,
                            events_path,
                        )
                    else:
                        _, _, event_record = _raw_event_stream(events_path)
    ...
    try:
        if event_retention == "compact":
            ...
            events, invalid_lines, event_record = _raw_event_stream(events_path)
        record.update(event_record)
    except OSError as exc:
    ...
    record.update(
        {
            ...
            "jsonl_event_count": (
                event_record["raw_jsonl_event_count"]
                if event_retention == "compact"
                else len(events)
            ),
