    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            return payload, None
        except Exception as exc:  # noqa: BLE001 - keep agent boundary robust.
            last_error = str(exc)
...
def request_fix_plan(
    ...
    *,
    ...
    fallback = {
        ...
        "source": main_input["source"],
        "plans": [],
        "unplanned_tasks": [
            {
                "task_index": str(summary.get("task", {}).get("task_index", "")),
                "task_name": str(summary.get("task", {}).get("task_name", "")),
                "reason": "main_agent_failed_to_emit_valid_fix_plan",
            }
            for summary in main_input.get("task_summaries", [])
        ],
        "generation_errors": [
            *main_input.get("generation_errors", []),
            {"stage": "main_agent", "error": last_error},
        ],
    }
    validate_fix_plan_set({**fallback, "plans": []})
    return fallback
