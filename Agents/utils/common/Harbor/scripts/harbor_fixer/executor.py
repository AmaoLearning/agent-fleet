...
def _unexecuted_command_record(
    output_dir: Path,
    plan_id: str,
...
def _run_command(
    ...
    try:
        ...
        return _unexecuted_command_record(
            output_dir,
            plan_id,
            command,
            command_index,
            cwd,
            status="failed",
            reason=f"command cwd is not accessible: {cwd}: {exc}",
        )
    if not cwd_exists:
        return _unexecuted_command_record(
            output_dir,
            ...
            cwd,
            status="failed",
            reason=f"command cwd does not exist or is not a directory: {cwd}",
        )
        ...
        for command_index, command in enumerate(plan["commands"]):
            if plan_failed:
                command_results.append(
                    _unexecuted_command_record(
                        output_dir,
                        plan_id,
                        command,
                        command_index,
                        _resolve_cwd(workspace_root, command["cwd"]),
                        status="skipped",
                        reason="previous command in this plan failed",
                    )
                )
                continue
