...
def validate_report_input(payload: dict[str, Any]) -> None:
    _check_kind(
        payload, version=1, kind="harbor_fixer_report_input", name="report input"
    )
    require_dict(payload.get("source"), "report input source")
    require_enum(
        payload.get("baseline_monitor_policy"),
        "baseline_monitor_policy",
        MONITOR_POLICIES,
    )
    validate_verification_result(
        require_dict(payload.get("verification_result"), "verification_result")
    )
...
def _validate_report_matches_verification(
    ...
    _require_equal(
        new_run.get("rerun"), verification_result.get("rerun"), "new_run.rerun"
    )
