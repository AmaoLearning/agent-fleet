    ...
    for task_input in task_inputs:
        ...
        for evidence in task_input.get("evidence", []):
            ...
            if not path_value:
                continue
            try:
                requested_start = max(1, int(evidence.get("line_start") or 1))
                requested_end = max(requested_start, int(evidence.get("line_end") or requested_start))
            ...
            try:
                entry["resolved_path"] = str(path.resolve(strict=True))
            except OSError:
                entry["resolved_path"] = ""
...
def collect_workspace_evidence(
    ...
    analyzer_artifacts = {
        name: _path_state(analyzer_output / name)
        for name in (
