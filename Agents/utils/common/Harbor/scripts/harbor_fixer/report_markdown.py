...
def _planned_action_text(action: dict[str, Any]) -> tuple[str, str]:
    if action.get("action_type") == "command":
        ...
        argv.extend(str(value) for value in action.get("arguments", []))
...
def render_human_report(
    ...
    *,
    ...
    lines = [
        ...
        "",
        _bounded_human_text(summary.get("text"))
        ...
        for action in plan.get("actions", []):
            ...
            if evidence:
                lines.extend(
                    [
                        "",
                        f"**{plan.get('plan_id', '-')}/{action.get('action_id', '-')} failure evidence**",
                        "",
    ...
    lines.extend(["", "### Recorded failures and interruptions", ""])
    ...
    for plan_id, plan in plans.items():
        ...
        lines.extend(
            [
                ...
                "**Plan summary**",
                "",
                _bounded_human_text(fix_reason.get("summary"))
