...
def _old_task_by_index(report: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {
        str(item.get("task", {}).get("task_index") or ""): item
...
def _fallback_summary_text(summary_input: dict[str, Any]) -> tuple[str, list[str]]:
    verification_status = str(summary_input.get("status") or "inconclusive")
    old_run = summary_input.get("old_run") if isinstance(summary_input.get("old_run"), dict) else {}
    analyzer_summary = old_run.get("analyzer_summary") if isinstance(old_run.get("analyzer_summary"), dict) else {}
    new_run = summary_input.get("new_run") if isinstance(summary_input.get("new_run"), dict) else {}
    new_run_summary = new_run.get("summary") if isinstance(new_run.get("summary"), dict) else {}

    task_count = int(analyzer_summary.get("task_count") or 0)
    env_infra_task_count = int(old_run.get("env_infra_task_count") or 0)
    sampled_task_count = int(new_run_summary.get("sampled_task_count") or 0)
    unsampled_task_count = int(new_run_summary.get("unsampled_task_count") or 0)
    complete_success = int(new_run_summary.get("complete_success") or 0)
    complete_failed = int(new_run_summary.get("complete_failed") or 0)
    complete_unknown = int(new_run_summary.get("complete_unknown") or 0)
    not_complete = int(new_run_summary.get("not_complete") or 0)

    text = (
        f"Deterministic fallback summary: verification finished with status "
        f"`{verification_status}` for {task_count} analyzed task(s). "
        f"The Analyzer marked {env_infra_task_count} task(s) as env/infra. "
        f"The new run evaluated {sampled_task_count} sampled task(s)"
    )
    if unsampled_task_count:
        text += f" and left {unsampled_task_count} unsampled"
    text += ". "
    text += (
        f"Observed outcomes: {complete_success} complete_success, "
        f"{complete_failed} complete_failed, {complete_unknown} complete_unknown, "
        f"{not_complete} not_complete."
    )

    highlights = [
        f"verification status: {verification_status}",
        f"analyzer env/infra tasks: {env_infra_task_count}/{task_count}",
        (
            "new run outcomes: "
            f"{complete_success} complete_success, {complete_failed} complete_failed, "
            f"{complete_unknown} complete_unknown, {not_complete} not_complete"
        ),
    ]
    if sampled_task_count or unsampled_task_count:
        highlights.append(
            f"smoke sampling: {sampled_task_count} sampled, {unsampled_task_count} unsampled"
        )
    return text, highlights


def _fallback_summary(summary_input: dict[str, Any], errors: list[dict[str, Any]]) -> dict[str, Any]:
    text, highlights = _fallback_summary_text(summary_input)
    return {
        "schema_version": 1,
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if not isinstance(payload.get("generation_errors"), list):
                payload["generation_errors"] = []
