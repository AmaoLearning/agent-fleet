...
Evaluate exactly one command whose write targets are deterministically contained by
explicitly user-authorized writable roots. These files may be environment configuration,
daemon configuration, or Dockerfiles rather than project source. Return JSON only and use
...
inside explicitly user-authorized writable roots. T3 commonly includes writes when no
root was authorized, repository source changes, Docker operations, dependency installation,
service or daemon operations, network operations, and commands with unknown or outside-root
effects. Return JSON only and use only the supplied input.

The writable_roots list is the authoritative boundary for direct, path-addressed host
filesystem writes. workspace_root is context only and never grants write permission. Deny
such a write unless every target is proven to be inside writable_roots; when writable_roots
is empty, deny every direct host-path write. Writes internal to Docker, package managers,
or service managers are managed-system effects and may be evaluated normally. If a managed
operation also writes, copies to, or mounts an explicit host path, apply the writable-root
boundary. Do not infer write permission from repository or workspace containment.
