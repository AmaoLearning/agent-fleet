...
def _rule_segments(
    ...
    tokens = _lex(command) if tokens is None else tokens
    ...
    for segment in segments:
        for nested in _nested_shell_commands(segment):
            nested_segments.extend(_rule_segments(nested))
    return [*segments, *nested_segments]


def _builtin_deny_reason(
    command: str,
    rule_segments: list[list[str]] | None = None,
) -> tuple[str, str] | None:
    try:
        segments = _rule_segments(command) if rule_segments is None else rule_segments
    except ValueError:
        return None
    for segment in segments:
        reason = builtin_destructive_reason(segment)
        if reason is not None:
            return reason
        if Path(segment[0]).name == "xargs" and segment[1:]:
            nested_reason = _builtin_deny_reason(" ".join(segment[1:]))
...
def evaluate_t1(command: str, user_rules: list[PrefixRule]) -> dict[str, Any] | None:
    try:
        tokens = _lex(command)
        rule_segments = _rule_segments(command, tokens)
    except ValueError:
        return None
    deny_reason = _builtin_deny_reason(command, rule_segments)
