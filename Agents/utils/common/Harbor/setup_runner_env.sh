...
HARBOR_RUNNER_SOURCE_URL="${HARBOR_RUNNER_SOURCE_URL:-https://github.com/harbor-framework/harbor.git}"
HARBOR_RUNNER_SOURCE_COMMIT="${HARBOR_RUNNER_SOURCE_COMMIT:-4407eb5227a2ff4f0d3f16b2eb48849382fdf276}"
...
validate_runner() {
  ...
  HARBOR_RUNNER_SOURCE_COMMIT="$HARBOR_RUNNER_SOURCE_COMMIT" \
...
UV_LINK_MODE=copy "$HARBOR_RUNNER_UV_BIN" pip install \
  ...
  "harbor @ git+${HARBOR_RUNNER_SOURCE_URL}@${HARBOR_RUNNER_SOURCE_COMMIT}"
UV_LINK_MODE=copy "$HARBOR_RUNNER_UV_BIN" pip install \
  --only-binary :all: \
  --python "$HARBOR_RUNNER_HOST_DIR/bin/python" \
