"""Harbor adapter for the pinned Ante runtime.

Derived from AntigmaLabs/ante ``ante-harbor`` at tag ``v0.preview.71``
(Apache-2.0). Agent Fleet adds Harbor 0.18 compatibility and a runner-cached
binary path so OpenSandbox trials do not install Ante from the internet.
"""
...
try:
    ...
    # Harbor 0.18.0 predates these three more specific exception classes.
    # Keep stable names in result.json while retaining its UnknownApiError
    # handling and retry semantics.
    class AgentAuthenticationError(UnknownApiError):
        pass

    class AgentSafetyRefusalError(UnknownApiError):
        pass

    class ContextWindowExceededError(UnknownApiError):
        pass
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._binary_path = Path(
            binary_path
            or os.environ.get("ANTE_BINARY_PATH", "")
            or Path(__file__).parent / "ante"
        ).expanduser()
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        ...
        if self._install_command is not None:
            ...
            if not binary_path.is_file():
                raise ValueError(f"Ante binary path is not a file: {binary_path}")
